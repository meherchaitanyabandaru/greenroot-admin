...
export function QuotationDetailPanel({ quotationId }: { quotationId: number }) {
  ...
  const [fetchRenderedPdf, { isFetching: isRenderedPdfFetching }] = useLazyGetQuotationRenderedPdfQuery();
  ...
  async function handleDownloadPdf() {
    try {
      const rendered = await fetchRenderedPdf(quotationId).unwrap();
      downloadBlob(rendered.blob, rendered.filename ?? `${text(q.quotation_code)}.pdf`);
      setPdfSnack({
        open: true,
        severity: 'success',
        msg: 'PDF generated by API.',
      });
    } catch {
      setPdfSnack({ open: true, severity: 'error', msg: 'Failed to generate PDF. Please try again.' });
    }
  }
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            startIcon={isRenderedPdfFetching ? <CircularProgress size={14} color="inherit" /> : <PictureAsPdfIcon sx={{ fontSize: 16 }} />}
            endIcon={!isRenderedPdfFetching && <DownloadIcon sx={{ fontSize: 14 }} />}
            onClick={handleDownloadPdf}
            disabled={isRenderedPdfFetching}
            sx={{ fontSize: 12 }}
          >
            {isRenderedPdfFetching ? 'Generating…' : 'Download PDF'}
          </Button>
        </Stack>
      </Stack>
